...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="barWidth" as="xs:integer" select="30"/>
    ...
    <xsl:template name="xsl:initial-template">
        <svg viewBox="50 -600 1050 600" width="100%">
            <text text-anchor="middle" x="{$xLength div 2}" y="-500" font-size="30">How are men, women, and mixed groups described across both editions?</text>
            ...
            <g transform="scale(.75) translate(150, -150)">
                ...
                <xsl:for-each select="(1 to ($yScale div 50))">
                    ...
                    <text x="-50" y="-{. * 50 - 5 + 210}" stroke="black" fill="black"
    ...
    <xsl:template match="$fairytales">
        <xsl:for-each select="$fairytales/*">
            ...
            <g>
                ...
                <rect x="{$pos}" y="-{$mixHeight + 210 + $mHeight + $fHeight}" width="{$barWidth}" height="{$mixHeight}" fill="#9999FF"/>
                ...
                <rect x="{$pos + $barWidth + $interSpace}" y="-{$hHeight + 210}" width="{$barWidth}" height="{$hHeight}" fill="#003300"/>
                ...
                <rect x="{$pos + $barWidth + $interSpace}" y="-{$nHeight + 210 + $hHeight}" width="{$barWidth}" height="{$nHeight}" fill="#4C9900"/>
                ...
                <rect x="{$pos + $barWidth + $interSpace}" y="-{$eHeight + 210 + $hHeight + $nHeight}" width="{$barWidth}" height="{$eHeight}" fill="#99FF33"/>
            </g>
        </xsl:for-each>
    </xsl:template>
</xsl:stylesheet>
